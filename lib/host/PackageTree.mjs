...
/// @brief Take the package.json and return the "scriptlet list". (The scriptlet list is, essentially
/// the js-hell key in the package: an object whose keys are the package name and whose values
/// are the IDL or '*'.)
///
/// FIXME: Scriptlet *List* seems like a misnomer. Something more appropriate.
function 
getScriptletList( packageJson, recurse = false, diagnostic_packageJsonUrl ) {
    const result = {};
...
addChildPackages( packageTree, url, { recurse = false } ) {
    const urlString = url.toString(),
          ...
          nestedPackageList = getScriptletList( packageJson, recurse, urlString );
    await packageTree.addScriptletList( nestedPackageList, url, packageJson );
    
    // 2024_10_15: NB `packageJSON` is returned as `undefined` is the import failed.
    // The functions above warn and cope. Hence the need for another test with `?.`.
    //
    // It would make sense to move the warning after `fetchJson()` - except the same code 
    // is used for childPackages and it makes ense to have it in getScriptletList().
    //
    // Q: Should we add the warning in `fetchJson()` and then check afterwards?  
